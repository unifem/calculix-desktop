# Docker Image for CalculiX
This Docker image provides the Ubuntu 17.10 environment with X Windows with CalculiX. The X Windows will display in your web browser in full-screen mode. You can use this Docker image on 64-bit Linux, Mac or Windows. It allows you to use the same programming environment regardless which OS you are running on your laptop or desktop.

[![Build Status](https://travis-ci.org/unifem/calculix-desktop.svg?branch=master)](https://travis-ci.org/unifem/calculix-desktop) [![](https://images.microbadger.com/badges/image/unifem/calculix-desktop.svg)](https://microbadger.com/images/unifem/calculix-desktop)
...
To run the Docker image, first download the script [`calculix_desktop.py`](https://raw.githubusercontent.com/unifem/calculix-desktop/master/calculix_desktop.py)
...
```
curl https://raw.githubusercontent.com/unifem/calculix-desktop/master/calculix_desktop.py -outfile calculix_desktop.py
```
On Linux or Mac, start a terminal, use the `cd` command to change to the working directory, and then run the following command:
```
curl -s -O https://raw.githubusercontent.com/unifem/calculix-desktop/master/calculix_desktop.py
```

After downloading the script, you can start the Docker image using the command
```
python calculix_desktop.py -p
```
...
```
python calculix_desktop.py -v myproject
```
and the work directory will be the data volume.

For additional command-line options, use the command
```
python calculix_desktop.py -h
```
...
default web browser on your computer. Simply replace `calculix_desktop.py` with `calculix_jupyter.py` in the preceding commands. That is, on Windows run the commands
```
curl https://raw.githubusercontent.com/unifem/calculix-desktop/master/calculix_jupyter.py -outfile calculix_jupyter.py
python calculix_jupyter.py -p
```
or on Linux and Mac run the commands
```
curl -s -O https://raw.githubusercontent.com/unifem/calculix-desktop/master/calculix_jupyter.py
python calculix_jupyter.py -p
```
...
```
python calculix_desktop.py
```
or
```
python calculix_jupyter.py
```
